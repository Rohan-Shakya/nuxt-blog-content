...
const { data: relatedArticles } = await useAsyncData(
  `content:related-articles:${page.title}`,
  () =>
    queryContent("/articles")
      .where({ categories: { $in: page.categories }, _extension: "md" })
      .only([
        "_path",
        "title",
        "categories",
...
<template>
  <UContainer>
    <UPage>
      ...
      <div
        class="mt-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[96px_768px_1fr]"
      >
        ...
        <div
          class="row-start-2 lg:row-start-1 lg:col-start-3 lg:px-8 space-y-8"
        >
          <UButton
            to="#"
            color="primary"
